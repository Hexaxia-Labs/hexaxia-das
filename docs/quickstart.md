...
(`0.4.0`) and the design spec version (`v0.3`). `org` is the optional org code from `--org`.
The `tags` block is the optional vocabulary built from each `--tag CODE=description` you passed;
codes are 2-5 characters (start with an uppercase letter, then uppercase letters and/or digits) and are the only tags `das new` will accept later. If you ran
...
das add 02.02 Pinnacle-Health "Pinnacle Health Systems - Springfield, IL"
```
...
```
  ...
  02.02                Pinnacle-Health                Pinnacle Health Systems - Springfield, IL
...
For the full passport field reference and how to fill in the stub, see [passports.md](passports.md).
...
"02.02":
  ...
  description: Pinnacle Health Systems - Springfield, IL
  type: category
  parent: "02"
  deprecated: true
```
