...
without a search. Hexaxia DAS files carry their address: `04.02.01-plan-business-roadmap.docx` is
...
- What is the corpus's org namespace, if any (used for federation references)?
- What tag vocabulary scopes files to clients, markets, or entities?
- What naming style does the corpus use - address-first, slug-date, or custom?
...
without traversing a single folder. It knows that `02` is Clients, that `02.01` is the ACME
...
Version 0.4.0 is a working proof of concept. The addressing system, manifest, naming
convention, validator, and document passports are in place; agent navigation is documented
(agent-guide.md) and multi-corpus federation is a documented, advisory convention
(federation.md) whose tooling enforcement is deferred. The standard is intentionally
minimal - stable enough to build on, open enough to extend.
...
| Multi-entity | No | Documented (federated `ORG:address` references; tooling enforcement deferred) |
...
Hexaxia DAS was designed to solve a specific, real problem: an organization managing a growing
...
## The Standard Is Flexible - But Only If You Think First

The permanence rule is what forces the thinking to happen upfront. You cannot renumber your
way out of a bad decision later, so you make a good decision now. But the standard itself
does not box you in. Two digits or three, sequential or gapped, one layout pattern or mixed,
two levels or five - all of it is valid as long as it is deliberate and declared. The corpus
conventions document becomes the contract that captures those decisions, so a future
maintainer or agent understands not just what the structure is, but why it is that way.

That is what scales: not just the address space, but the reasoning behind it.

---

## Many Corpora, Varying Standards

The standard does not require uniformity across corpora. Each corpus is self-contained: its
own config, its own conventions, its own naming style, its own segment width. The named-root
containment model means an agent operating in one corpus does not need to know anything about
how another corpus is laid out. Each root is an independent trust boundary.

A content editorial corpus running slug-date naming with a publish lifecycle can sit alongside
a sensor-telemetry corpus running das-address naming with three-digit segments, which sits
alongside a product corpus with gapped area numbering and an open-core edition model. None of
their conventions bleed into the others. What makes them all legible by the same tooling -
`das validate`, the manifest structure, the passport schema - is the standard. What makes each
one right for its purpose is the freedom to vary every design decision that should vary.

The standard is the floor. Everything above it is yours to decide.

---

*Hexaxia DAS v0.4.0 | Hexaxia Technologies | 2026-06-10*
