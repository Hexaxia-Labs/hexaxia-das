...
A query like "what blockers exist on the NetBird deployment for ULS?" will route to the right document if and only if the summary for that document answers that question semantically. The model does not read the document — it only sees the summary.

**Benchmark confirmation:** mxbai-embed-large routed 7/8 questions correctly in the test corpus. The one miss (Q4: "list all active ULS projects") was traced to a weak passport summary on the ULS Projects folder — its summary did not enumerate the projects it contained. This caused the model to route to a tangentially related document. The summary is the entire signal.
...
**Question an agent might ask:** "What blockers exist on the NetBird ZTNA deployment for ULS?"

**Poor:**
```
This document describes the deployment process for NetBird ZTNA at United Life Services.
It covers installation, configuration, and troubleshooting steps.
```

**Good:**
```
Six-phase runbook to deploy NetBird ZTNA for United Life Services (ULS). Current blockers:
firewall rules on ports 51820/UDP not yet approved by ULS IT, MFA enrollment pending for 3
admin accounts, and UAT sign-off deferred until May 30. Contact: Jay M. (ULS IT lead).
```

Why the good one works: it names the subject (NetBird ZTNA), the client (ULS), the document type (runbook), and the specific facts an agent would retrieve (blockers, port numbers, personnel, deadline). The poor one is accurate but generic — "deployment process" and "troubleshooting steps" could describe dozens of documents.

---

### Enumeration (Q4-class / D1-class)

**Question an agent might ask:** "List all active ULS projects."

**Poor (on the ULS Projects folder):**
```
Contains project documents for the United Life Services client engagement.
```

**Good:**
```
ULS active projects (4): NetBird ZTNA deployment (02.01.04, runbook), security compliance
audit (02.01.06, report, Q3 target), staff onboarding automation (02.01.07, plan, draft),
and web portal redesign (02.01.09, spec, on hold). All project files live in 02.01-ULS/.
```
...
**Question an agent might ask:** "What deliverables have been sent to Pax Nocturna in the last 60 days?"

**Poor:**
```
Deliverable tracking for Pax Nocturna. Includes proposals, reports, and other client documents.
```

**Good:**
```
Deliverable log for Pax Nocturna (PN) client engagement. Last 60 days: brand identity
...
<!--
passport:
  title: "02.01-ULS Active Projects Index"
  type: reference
  status: active
  tags: [uls]
  das_address: "02.01"
  summary: "Active projects for United Life Services: NetBird ZTNA (02.01.04, runbook,
            ...
            Jay M. Primary address: 02.01-ULS/."
-->
```

This one passport summary answers the Q4-class question ("list all active ULS projects") from RAG alone, with zero navigation turns.
...
The benchmark miss on Q4 (ULS active projects) is the clearest example of what happens when step 3 fails. The summary for `02.01-ULS` said something like "United Life Services client folder" — it contains the word ULS but says nothing about projects or their statuses. The model routed to a LinkedIn Scheduled folder instead because that document's summary mentioned "active posts" — which overlapped with "active projects" more than the ULS index did.
...
passport:
  ...
  tags: [uls, netbird]    # from corpus vocabulary in das.config.yaml
